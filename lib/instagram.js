...
async function publishFeedPost({ content, mediaUrl, accessToken, igUserId }) {
    ...
    // Wait for ALL media to be ready (images AND videos)
 ...
 */
async function waitForMediaReady(containerId, accessToken, maxTries = 20) {
    for (let i = 0; i < maxTries; i++) {
        ...
        console.log('[Instagram] Media status:', JSON.stringify(statusData))

        if (statusData.status_code === 'FINISHED') return true
        if (statusData.status_code === 'READY') return true
